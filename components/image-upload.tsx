...
async function compressImageIfNeeded(
  ...
  try {
    ...
    if (scale === 1 && blob.size >= file.size) return { file, compressed: false }
...
export async function uploadImageFile(
  ...
  if (processed.file.size > maxSizeBytes) {
    throw new Error(`图片超过大小限制：${formatBytes(processed.file.size)} / ${formatBytes(maxSizeBytes)}`)
  }
    ...
    <div className={cn('relative space-y-2', className)}>
      ...
      {preview ? (
        <div className="relative inline-block">
          <img src={preview} alt="Preview" className="max-h-32 rounded border border-border/50" />
          {uploading && (
        ...
        <button
          ...
          className={cn(
            ...
            compact ? 'h-8 rounded-md px-3 text-xs' : 'rounded-lg border-2 border-dashed px-4 py-3 text-sm',
            dragOver
      ...
      {lastUpload && !compact && (
        <p className="text-xs text-muted-foreground">
          ...
          {lastUpload.compressed ? `，已从 ${formatBytes(lastUpload.originalSize)} 自动压缩` : ''}
        </p>
      )}
      ...
      {error && (
        <p className={cn(
          ...
          compact && 'absolute left-0 top-full z-10 mt-2 w-64 rounded-md border border-destructive/30 bg-background px-2 py-1.5 shadow-sm',
        )}>
...
export function ImageUploadInline({
  ...
  return (
    <div className="relative">
      ...
      {error && <p className="absolute left-0 top-full mt-1 w-64 text-xs text-destructive">{error}</p>}
    </div>
  )
}
