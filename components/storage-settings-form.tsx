...
import type { StorageProviderEnvironmentStatus, StorageProviderName } from '@/lib/storage'
import {
  formatStorageBytes,
...
export function StorageSettingsForm({
  ...
  const selectedStatus = providerStatuses.find((status) => status.provider === provider)
  const showInferredQuota = quota === 0 && provider === activeProvider && inferredQuotaBytes !== null
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="space-y-2">
        <Label>上传存储后端</Label>
        <Select value={provider} onValueChange={(value) => setProvider(value as StorageProviderName)}>
          <SelectTrigger className="w-full">
            <SelectValue placeholder="选择存储后端" />
          </SelectTrigger>
          <SelectContent>
            {providerStatuses.map((status) => (
              <SelectItem key={status.provider} value={status.provider} disabled={!status.configured}>
                {status.label}{status.configured ? '' : '（未配置）'}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
