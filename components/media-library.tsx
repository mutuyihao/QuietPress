...
export function MediaLibrary({
  ...
  const usageText =
    showUsage && usage?.usedBytes !== null && usage?.usedBytes !== undefined
  ...
  const reduceUsageAfterDelete = (file: StoredFile) => {
    setUsage((current) => {
      ...
      return {
        ...
        objectCount,
        availableBytes:
          current.quotaBytes !== null && usedBytes !== null
  ...
  const deleteFile = (file: StoredFile) => {
    ...
    startTransition(async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(
            getApiErrorMessage(await res.json().catch(() => null), "删除失败"),
          );
        }
        setFiles((current) =>
      ...
      {files.length === 0 ? (
        ...
        >
          {files.map((file) => (
            <div
              key={file.path}
              className="admin-panel group overflow-hidden rounded-lg"
            >
              <button
                type="button"
                onClick={() =>
