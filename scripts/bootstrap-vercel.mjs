...
async function findUserByEmail(supabase, email) {
  ...
  throw new Error(
    "Could not find existing ADMIN_EMAIL user within the first 2000 Supabase Auth users.",
  );
}
...
async function runMigration(databaseUrl) {
  ...
  try {
    ...
    const [{ locked }] =
      await sql`select pg_try_advisory_lock(${MIGRATION_LOCK_ID}) as locked`;
    if (!locked) {
      throw new Error(
        "Another QuietPress bootstrap migration is already running. Retry after that deployment finishes.",
      );
    }
    ...
    for (const fileName of migrationFiles) {
      ...
      if (existing[0]?.checksum === migrationChecksum) {
        console.log(
          `[bootstrap] Skipping migration ${fileName}; already applied.`,
        );
        continue;
      }
      ...
      `;
      console.log(
        `[bootstrap] Applied migration ${fileName} in ${formatDuration(startedAt)}.`,
      );
    }
...
async function ensureInitialAdmin() {
  ...
  if (existingAdmins.length > 0) {
    console.log(
      "[bootstrap] Admin profile already exists; password will not be reset.",
    );
    return;
  }
