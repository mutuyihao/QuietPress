...
export class SupabaseStorage implements StorageProvider {
  ...
  async getUsage(): Promise<{
    ...
    const walk = async (prefix = ''): Promise<void> => {
      ...
      while (true) {
        ...
        if (error) throw new Error(`Supabase usage scan failed: ${error.message}`)
  ...
  private async getUsageFromRpc(): Promise<{
    ...
    const { data, error } = await this.supabase.rpc('get_storage_bucket_usage', {
  ...
  async listFiles(): Promise<StoredFile[]> {
    ...
    const walk = async (prefix = ''): Promise<void> => {
      ...
      while (true) {
        ...
        if (error) throw new Error(`Supabase media scan failed: ${error.message}`)
        if (!data || data.length === 0) break
        ...
        for (const item of data) {
          ...
          if (Number.isFinite(size) && size >= 0) {
            files.push({
              ...
              name: item.name,
              size,
              contentType: typeof item.metadata?.mimetype === 'string'
