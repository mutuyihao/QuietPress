...
export async function getStorageDashboard(settings: SiteSettings | null): Promise<StorageDashboard> {
  const activeProvider = normalizeStorageProvider(settings?.storage_provider || process.env.STORAGE_PROVIDER || 'supabase')
  const providerStatus = getStorageProviderEnvironmentStatus(activeProvider)
  const quotaMb = Number(settings?.storage_quota_mb || 0)
  const defaultQuota = quotaMb > 0 ? null : getDefaultStorageQuota(activeProvider)
  const effectiveQuotaMb = quotaMb > 0 ? quotaMb : defaultQuota?.quotaMb ?? 0
  const quotaBytes = effectiveQuotaMb > 0 ? effectiveQuotaMb * BYTES_PER_MB : null
  const quotaSource = quotaMb > 0 ? 'manual' : defaultQuota?.source ?? null
  const configuredMaxUploadBytes = getImageUploadMaxSizeBytes(getImageUploadConfig(settings))
  ...
  const maxUploadBytes = bucketFileSizeLimitBytes !== null
  ...
  return {
    ...
    quotaBytes,
    quotaSource,
    availableBytes: quotaBytes !== null && usedBytes !== null ? Math.max(0, quotaBytes - usedBytes) : null,
    maxUploadBytes,
