...
const getPublishedPostsCached = unstable_cache(async (page = 1, pageSize = 10): Promise<PaginatedResult<PostWithTags>> => {
...
const getPostBySlugCached = unstable_cache(async (slug: string): Promise<PostWithTags | null> => {
  const { posts } = await getPublicRepos()
...
export const getPostBySlug = cache((slug: string) => getPostBySlugCached(slug))

const getPostsByTagCached = unstable_cache(async (tagSlug: string): Promise<PostWithTags[]> => {
  const { posts } = await getPublicRepos()
...
export const getPostsByTag = cache((tagSlug: string) => getPostsByTagCached(tagSlug))

const getAllTagsCached = unstable_cache(async (): Promise<Tag[]> => {
...
const getTagBySlugCached = unstable_cache(async (slug: string): Promise<Tag | null> => {
  const { tags } = await getPublicRepos()
...
const getSiteSettingsCached = unstable_cache(async (): Promise<SiteSettings | null> => {
