...
import { addMediaItem, isHttpUrl, parseMarkdownImageUrls } from '@/lib/migration/utils'
...
export async function createQuietPressExport(supabase: SupabaseClient): Promise<QuietPressExportV1> {
  ...
  const exportedSettings = settings
    ? {
        ...
        base_url: settings.base_url && isHttpUrl(settings.base_url)
  ...
  return {
    meta: {
      ...
      source_url: exportedSettings?.base_url || normalizeSiteUrl(getDefaultSiteUrl()),
      counts: {
