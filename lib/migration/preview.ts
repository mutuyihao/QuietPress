...
import type { MigrationPreview, QuietPressExportV1 } from '@/lib/migration/types'
...
export async function createMigrationPreview(
  ...
  const postSlugs = Array.from(new Set(migrationPackage.posts.map((post) => post.slug)))
  const tagSlugs = Array.from(new Set(migrationPackage.tags.map((tag) => tag.slug)))
  ...
  const invalidMediaUrls = migrationPackage.media.filter((item) => !isSafeRemoteMediaUrl(item.url)).length
  const warnings: string[] = []

  if (invalidMediaUrls > 0) {
    warnings.push(`${invalidMediaUrls} media URL(s) are not safe public HTTP(S) URLs and will not be re-imported.`)
  }
