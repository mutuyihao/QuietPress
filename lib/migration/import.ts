...
function collectPackageMediaUrls(migrationPackage: QuietPressExportV1): string[] {
...
async function fetchMediaBuffer(url: string, maxSizeBytes: number): Promise<{ buffer: Buffer; contentType: string }> {
  ...
  if (Number.isFinite(contentLength) && contentLength > maxSizeBytes) {
    throw new Error(`Remote file is larger than ${Math.round(maxSizeBytes / 1024 / 1024)}MB`)
  }
  ...
  if (!detectedMime || !getAllowedImportImageMimeTypes().includes(detectedMime)) {
...
async function importOneMedia(
  ...
  if (!isSafeRemoteMediaUrl(url)) {
    return { originalUrl: url, error: 'Skipped unsafe or non-public media URL' }
  }

  const { buffer, contentType } = await fetchMediaBuffer(url, context.maxSizeBytes)
  ...
  const filename = sanitizeFilenameFromUrl(url, contentType)
  const upload = await context.provider.upload(buffer, filename, contentType, context.folder)
...
async function importMedia(
  ...
  try {
    ...
    return {
      ...
      failed: collectPackageMediaUrls(migrationPackage).map((url) => ({
        originalUrl: url,
        error: error instanceof Error ? error.message : 'Storage backend is not available',
      })),
      skipped: [],
      cleanupPaths: [],
      cleanupProvider: null,
  ...
  for (const url of collectPackageMediaUrls(migrationPackage)) {
    try {
      const result = await importOneMedia(url, context)
      if (result.importedUrl && result.path) {
        uploaded.push(result)
        cleanupPaths.push(result.path)
        urlMap[url] = result.importedUrl
      } else {
        skipped.push(result)
      }
    } catch (error) {
...
async function cleanupUploadedMedia(provider: StorageProvider | null, paths: string[]): Promise<void> {
...
export async function importQuietPressPackage(
  ...
  const media = await importMedia(supabase, migrationPackage, options.importMedia)
