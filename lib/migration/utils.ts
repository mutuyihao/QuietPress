...
import type { QuietPressExportMedia, QuietPressExportV1 } from '@/lib/migration/types'
import { MAX_IMPORT_JSON_BYTES, quietPressExportV1Schema } from '@/lib/migration/types'
...
export function sanitizeFilenameFromUrl(value: string, contentType: string): string {
  const url = new URL(value)
...
export function remapPackageMediaUrls(
  ...
  const replacements = Object.entries(urlMap).filter(([from, to]) => from && to && from !== to)
  ...
  return {
    ...
    settings: source.settings
      ? {
          ...source.settings,
          default_og_image_url: replaceValue(source.settings.default_og_image_url),
        }
      : null,
    posts: source.posts.map((post) => ({
      ...post,
      content_markdown: replaceValue(post.content_markdown) ?? post.content_markdown,
...
export async function parseQuietPressPackageFromRequest(request: Request): Promise<QuietPressExportV1> {
  const text = await request.text()
  const bytes = Buffer.byteLength(text, 'utf8')

  if (bytes > MAX_IMPORT_JSON_BYTES) {
    throw new Error(`Import package is too large. Maximum size is ${Math.round(MAX_IMPORT_JSON_BYTES / 1024 / 1024)}MB.`)
  }
  ...
  const candidate = raw && typeof raw === 'object' && 'package' in raw
