...
type ImageUploadSettingsSource = Partial<
  Pick<
    SiteSettings,
...
export function getImageUploadConfig(
  settings?: ImageUploadSettingsSource | null,
): ImageUploadConfig {
  return {
    ...
    compressionEnabled:
      settings?.image_compression_enabled ??
      DEFAULT_IMAGE_UPLOAD_CONFIG.compressionEnabled,
    ...
    maxWidth: clampNumber(
      ...
      640,
      4096,
      DEFAULT_IMAGE_UPLOAD_CONFIG.maxWidth,
    ),
    maxHeight: clampNumber(
      settings?.image_max_height,
      640,
      4096,
      DEFAULT_IMAGE_UPLOAD_CONFIG.maxHeight,
    ),
  };
}
