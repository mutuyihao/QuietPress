...
export const MCP_DEFAULT_GRANT_TYPES = [
  "authorization_code",
  "refresh_token",
] as const;
...
export function normalizeOAuthRedirectUris(
...
export function isRegisteredRedirectUri(
  ...
  return registeredRedirectUris.includes(
    normalizeOAuthRedirectUri(redirectUri),
  );
}

export function normalizeOAuthScopes(
  value: string | null | undefined,
  fallback: readonly McpScope[] = MCP_SCOPES,
): McpScope[] {
  ...
  const requested = splitScopeString(value).filter(
