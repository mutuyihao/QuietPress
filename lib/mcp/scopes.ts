...
export function parseScopeString(value: string | null | undefined): McpScope[] {
  return Array.from(
    new Set(
      splitScopeString(value).filter((scope): scope is McpScope =>
        MCP_SCOPE_SET.has(scope),
      ),
    ),
  );
}

export function getUnknownMcpScopes(
  value: string | null | undefined,
): string[] {
  return splitScopeString(value).filter(
    (scope) =>
      !MCP_SCOPE_SET.has(scope) && !OAUTH_COMPATIBILITY_SCOPES.has(scope),
  );
}
...
export function normalizeScopes(scopes: Iterable<string>): McpScope[] {
  return Array.from(
    new Set(
      Array.from(scopes).filter((scope): scope is McpScope =>
        MCP_SCOPE_SET.has(scope),
      ),
    ),
  );
