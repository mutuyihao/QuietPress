...
import { migrationImportOptionsSchema, quietPressExportV1Schema } from '@/lib/migration/types'
...
const searchPostsSchema = z.object({
  ...
  status: z.union([postStatus, z.literal('all')]).optional(),
...
const createDraftSchema = z.object({
  ...
  tag_slugs: z.array(z.string().min(1).max(200)).optional(),
})
...
const updateSettingsSchema = dangerousSchema.extend({
  ...
  image_upload_max_size_mb: z.number().int().min(1).max(10).optional(),
  image_compression_enabled: z.boolean().optional(),
  image_compression_quality: z.number().int().min(40).max(95).optional(),
  image_max_width: z.number().int().min(640).max(4096).optional(),
  image_max_height: z.number().int().min(640).max(4096).optional(),
...
export const MCP_TOOLS: McpToolDefinition[] = [
  {
    ...
    scopes: ['posts:read'],
    inputSchema: schema({
      query: stringProp('Optional query matched against title, slug, excerpt, and markdown.'),
      status: { type: 'string', enum: ['draft', 'scheduled', 'published', 'archived', 'all'] },
      limit: { type: 'number', minimum: 1, maximum: 100 },
    }),
    parse: parseWith(searchPostsSchema),
    handler: (context, args) => searchBlogPosts(context.blog, args as z.infer<typeof searchPostsSchema>),
  },
  {
    name: 'blog.get_post',
    ...
    handler: (context, args) => getBlogPost(context.blog, args as z.infer<typeof getPostSchema>),
  },
  {
    ...
    handler: (context, args) => createBlogPostDraft(context.blog, args as z.infer<typeof createDraftSchema>),
  },
  {
    ...
    handler: (context, args) => updateBlogPost(context.blog, args as z.infer<typeof updatePostSchema>),
  },
  {
    ...
    inputSchema: schema({
      ...
      idempotency_key: stringProp('Caller-provided unique key for this unsafe action.'),
    }, ['id', 'confirm', 'idempotency_key']),
    parse: parseWith(postIdDangerousSchema),
    handler: (context, args) => setBlogPostStatus(context.blog, (args as z.infer<typeof postIdDangerousSchema>).id, 'published'),
  },
  {
    ...
    parse: parseWith(postIdDangerousSchema),
    handler: (context, args) => setBlogPostStatus(context.blog, (args as z.infer<typeof postIdDangerousSchema>).id, 'archived'),
  },
  {
    ...
    parse: parseWith(postIdDangerousSchema),
    handler: (context, args) => deleteBlogPost(context.blog, (args as z.infer<typeof postIdDangerousSchema>).id),
  },
  {
    name: 'blog.list_tags',
  ...
  },
  {
    ...
    handler: (context, args) => manageBlogTag(context.blog, args as z.infer<typeof manageTagsSchema>),
  },
  {
    ...
    handler: (context, args) => listBlogComments(context.blog, args as z.infer<typeof listCommentsSchema>),
  },
  {
    ...
    handler: (context, args) => moderateBlogComment(context.blog, args as z.infer<typeof moderateCommentSchema>),
  },
  {
    name: 'blog.upload_media_from_url',
    description: 'Fetch a public HTTP(S) image and upload it to the active storage provider.',
    scopes: ['media:write'],
    inputSchema: schema({
    ...
    handler: (context, args) => uploadBlogMediaFromUrl(context.blog, args as z.infer<typeof uploadMediaSchema>),
  },
  {
  ...
  },
  {
    ...
    description: 'Update allowlisted public site settings. Does not expose storage secrets.',
    ...
    inputSchema: schema({
      ...
      image_compression_enabled: booleanProp('Whether client compression is enabled.'),
  ...
  },
  {
    ...
    inputSchema: schema({
      package: { type: 'object', description: 'QuietPress export v1 package.' },
    }, ['package']),
    parse: parseWith(previewImportSchema),
    handler: (context, args) => previewBlogMigrationPackage(context.blog, (args as z.infer<typeof previewImportSchema>).package),
  },
  {
    name: 'blog.import_package',
    description: 'Import a QuietPress migration package. Requires confirmation and idempotency key.',
    scopes: ['migration:write'],
    inputSchema: schema({
      package: { type: 'object', description: 'QuietPress export v1 package.' },
      options: { type: 'object', description: 'Migration conflict and media options.' },
      confirm: booleanProp('Must be true.'),
      idempotency_key: stringProp('Caller-provided unique key for this unsafe action.'),
    ...
    handler: (context, args) => {
      ...
      return importBlogMigrationPackage(context.blog, parsed.package, parsed.options)
    },
  },
  {
...
export function listMcpTools(access: McpAccessContext) {
  return MCP_TOOLS
    .filter((tool) => hasEveryScope(access.scopes, tool.scopes))
    .map((tool) => ({
      name: tool.name,
...
export const MCP_RESOURCES = [
  ...
  },
  {
    ...
    name: 'Media',
    description: 'Media files from the active storage provider when listing is supported.',
    mimeType: 'application/json',
    scopes: ['posts:read'] as McpScope[],
  },
  {
...
export function listMcpResources(access: McpAccessContext) {
  return MCP_RESOURCES
    .filter((resource) => hasEveryScope(access.scopes, resource.scopes))
    .map((resource) => ({
      uri: resource.uri,
      name: resource.name,
