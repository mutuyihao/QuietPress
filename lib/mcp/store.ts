...
function mapClient(row: Record<string, unknown>): McpClientRecord {
  const registrationType =
    row.registration_type === "dynamic" ? "dynamic" : "manual";
  ...
  return {
    ...
    token_endpoint_auth_method:
      typeof row.token_endpoint_auth_method === "string"
        ? row.token_endpoint_auth_method
        : MCP_TOKEN_AUTH_METHOD,
    grant_types:
      toStringArray(row.grant_types).length > 0
        ? toStringArray(row.grant_types)
        : [...MCP_DEFAULT_GRANT_TYPES],
    response_types:
      toStringArray(row.response_types).length > 0
        ? toStringArray(row.response_types)
        : [...MCP_DEFAULT_RESPONSE_TYPES],
    client_metadata:
      row.client_metadata && typeof row.client_metadata === "object"
...
function mapToken(row: Record<string, unknown>): McpTokenRecord {
  const client =
    row.mcp_oauth_clients && typeof row.mcp_oauth_clients === "object"
...
export async function getMcpEnabled(
...
export async function createMcpClient(
  ...
  const scopes = assertKnownScopes(
    input.scopes.length > 0 ? input.scopes : [...MCP_SCOPES],
  );
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      token_endpoint_auth_method:
        input.tokenEndpointAuthMethod || MCP_TOKEN_AUTH_METHOD,
...
export async function revokeMcpTokenByRawToken(
  supabase: SupabaseClient,
  token: string,
): Promise<void> {
...
export async function exchangeAuthorizationCode(
  ...
  if (codeRow.client_id !== input.clientId)
  ...
  if (codeRow.resource !== input.resource)
...
export async function rotateRefreshToken(
  ...
  if (existing.client && !existing.client.enabled)
    throw new Error("OAuth client disabled");
  ...
  if (adminError || !adminProfile)
    throw new Error("Admin access has been revoked");
...
export async function listMcpAuditLogs(
  ...
  return (data || []).map((row) => ({
    ...
    user_agent_hash:
      typeof row.user_agent_hash === "string" ? row.user_agent_hash : null,
...
export function requestedScopesWithinClient(
  requested: string,
