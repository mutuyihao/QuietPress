...
function getPromptParams(params: unknown): {
  name: string;
...
> {
  if (
    ...
    return {
      response: NextResponse.json(
        { error: "Bearer tokens must be sent in the Authorization header." },
        { status: 400 },
      ),
    };
  }

  const anonymousLimit = await checkRateLimitForRequest(request, {
    scope: "mcp-auth",
    windowMs: 60_000,
    maxRequests: 60,
  });
  ...
  const access = await validateMcpAccessToken(
    service,
    token,
    getResourceUrl(request),
  );
  if (!access)
    return { response: unauthorized(request, "Invalid or expired token") };

  const clientLimit = await checkRateLimitIdentifier(
    `${access.clientId}:${access.userId}`,
    {
      scope: "mcp-client",
      windowMs: 60_000,
      maxRequests: 120,
...
async function handleRpc(
  ...
  try {
    ...
    if (message.method === "tools/call") {
      ...
      try {
        ...
        await insertMcpAuditLog(service, {
          ...
          requestId,
          ipHash: hashSensitiveValue(ip, "mcp-ip"),
          userAgentHash: hashSensitiveValue(userAgent, "mcp-ua"),
          status: "success",
        });
    ...
    if (message.method === "resources/read") {
      return ok(
        message.id,
        await readMcpResource(context, getResourceUri(message.params)),
      );
    }
    ...
    if (error instanceof McpScopeError) {
      return rpcError(message.id, -32003, error.message, {
    ...
    return rpcError(message.id, -32000, "MCP request failed");
  }
}

export const GET = withApiRoute("mcp.GET", async (request: NextRequest) => {
