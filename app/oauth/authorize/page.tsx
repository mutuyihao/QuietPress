...
export default async function OAuthAuthorizePage({
  ...
  if (!session) {
    ...
    login.searchParams.set(
      "next",
      getAuthorizePath({ ...(params || {}), resource }),
    );
  ...
  try {
    ...
    error =
      err instanceof Error ? err.message : "Invalid authorization request.";
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <Card className="w-full max-w-2xl">
        <CardHeader>
          ...
          <CardDescription>
            Review the OAuth request before allowing this MCP client to operate
            QuietPress.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-5">
          {error ? (
            ...
            <>
              ...
              <form
                action={approveMcpAuthorizationAction}
