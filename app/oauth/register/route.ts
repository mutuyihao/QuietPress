...
import { createMcpClient, getMcpEnabled, scopesToString } from '@/lib/mcp/store'
import {
...
function normalizeClientName(value: string | null): string {
  ...
  if (name.length > MAX_CLIENT_NAME_LENGTH) {
    throw new Error(`client_name must be ${MAX_CLIENT_NAME_LENGTH} characters or shorter.`)
  }
  return name
}
...
function normalizeContacts(metadata: DcrMetadata): string[] {
  ...
  if (contacts.length > MAX_CONTACTS) throw new Error(`contacts supports at most ${MAX_CONTACTS} entries.`)
  for (const contact of contacts) {
    if (contact.length > MAX_CONTACT_LENGTH) throw new Error(`contacts entries must be ${MAX_CONTACT_LENGTH} characters or shorter.`)
  }
...
function normalizeGrantTypes(metadata: DcrMetadata): string[] {
  ...
  if (value === undefined || value === null) return [...MCP_DEFAULT_GRANT_TYPES]
  const grantTypes = readStringArray(metadata, 'grant_types')
  const unsupported = grantTypes.filter((grantType) => !MCP_DEFAULT_GRANT_TYPES.includes(grantType as (typeof MCP_DEFAULT_GRANT_TYPES)[number]))
  if (unsupported.length > 0) throw new Error(`Unsupported grant_type: ${unsupported.join(', ')}`)
  if (!grantTypes.includes('authorization_code')) throw new Error('grant_types must include authorization_code.')
...
function normalizeResponseTypes(metadata: DcrMetadata): string[] {
  ...
  if (value === undefined || value === null) return [...MCP_DEFAULT_RESPONSE_TYPES]
  const responseTypes = readStringArray(metadata, 'response_types')
  const unsupported = responseTypes.filter((responseType) => !MCP_DEFAULT_RESPONSE_TYPES.includes(responseType as (typeof MCP_DEFAULT_RESPONSE_TYPES)[number]))
  if (unsupported.length > 0) throw new Error(`Unsupported response_type: ${unsupported.join(', ')}`)
  if (!responseTypes.includes('code')) throw new Error('response_types must include code.')
...
export async function POST(request: NextRequest) {
  ...
  if (contentLength > MAX_DCR_BODY_BYTES) {
    return dcrError('invalid_request', 'Dynamic client registration body is too large.', 413)
  }
  ...
  if (!limit.allowed) {
    return NextResponse.json(
      { error: 'rate_limited', error_description: 'Too many dynamic client registration requests.' },
      {
        status: 429,
  ...
  try {
    ...
    if (!enabled) return dcrError('temporarily_unavailable', 'Remote MCP is disabled.', 503)

    const metadata = await request.json() as DcrMetadata
    if (!metadata || typeof metadata !== 'object' || Array.isArray(metadata)) {
      return dcrError('invalid_request', 'Registration metadata must be a JSON object.')
    }

    const redirectUris = normalizeOAuthRedirectUris(readStringArray(metadata, 'redirect_uris'))
    const grantTypes = normalizeGrantTypes(metadata)
    const responseTypes = normalizeResponseTypes(metadata)
    const tokenEndpointAuthMethod = normalizeTokenEndpointAuthMethod(readOptionalString(metadata, 'token_endpoint_auth_method'))
    const scopes = normalizeOAuthScopes(readOptionalString(metadata, 'scope'), MCP_SCOPES)
    ...
    const client = await createMcpClient(service, {
      ...
      clientUri: normalizeHttpsUrl(readOptionalString(metadata, 'client_uri'), 'client_uri'),
      logoUri: normalizeHttpsUrl(readOptionalString(metadata, 'logo_uri'), 'logo_uri'),
    ...
    return dcrError(
      ...
      error instanceof Error ? error.message : 'Invalid dynamic client registration request.',
    )
  }
}
