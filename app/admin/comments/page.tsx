...
export default function AdminCommentsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"pending" | "approved" | "spam">(
    "pending",
  );
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/comments?status=${activeTab}`)
      .then((res) =>
        readApiJson<{ comments: CommentWithPost[]; message?: string }>(res),
      )
      .then((data) => {
        if (cancelled) return;
  ...
  const handleAction = (id: string, action: "approved" | "spam" | "delete") => {
    startTransition(async () => {
      try {
        if (action === "delete") {
          await readApiJson<{ deleted: boolean }>(
            await fetch(`/api/admin/comments?id=${id}`, { method: "DELETE" }),
          );
        } else {
          await readApiJson<{ updated: boolean }>(
  ...
  return (
    <div className="admin-page">
      ...
      <div className="admin-tabs" role="tablist" aria-label="评论状态筛选">
        {[
