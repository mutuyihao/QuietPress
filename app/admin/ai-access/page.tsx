        ...
        <div className="space-y-6">
              ...
              <form action={setMcpEnabledAction} className="flex flex-wrap items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    MCP 端点：<code>/api/mcp</code> - DCR：<code>/oauth/register</code>
                  </p>
                </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                手动客户端使用 Authorization Code + PKCE；标准 MCP 客户端也可以通过 Dynamic Client Registration 自动注册。
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div key={client.client_id} className="rounded-lg border border-border p-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div className="min-w-0 space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        注册方式：{client.registration_type === 'dynamic' ? '动态注册' : '手动创建'}
                      </p>
              ...
              {data?.tokens.length === 0 ? (
                ...
                <div key={token.id} className="flex flex-col gap-3 rounded-lg border border-border p-4 md:flex-row md:items-center md:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      过期时间 {formatDate(token.expires_at)} - 最近使用 {formatDate(token.last_used_at)}
                    </p>
                ...
                <div key={log.id} className="rounded-lg border border-border px-3 py-2 text-sm">
                  <div className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
                    <p className="font-medium text-foreground">
                      {log.tool_name} - {log.status === 'success' ? '成功' : '失败'}
                    </p>
